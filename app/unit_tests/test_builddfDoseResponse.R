...
#' Test builddfDoseResponse function in build_dataframes.R
#' @example builddfDoseResponse(annotation, symbol, connection)

# Test with single gene - Symbol
test_that("single gene - symbol", {
  df <- builddfDoseResponse("Symbol", "Cyp1a1", dbconn)
  expected_colnames <- c("Project_ID", "Longname", "Dose", "FoldChange", "Species_common",
...
test_that("multiple genes - symbol", {
  expect_error(builddfDoseResponse("Symbol", c("Cyp1a1", "Cyp1a2"), dbconn))
})

# Test with no genes - Symbol
test_that("no genes - symbol", {
  df <- builddfDoseResponse("Symbol", "", dbconn)
  expect_equal(nrow(df), 0)
})

# Test with single gene - Ensembl_ID
test_that("single gene - Ensembl_ID", {
  df <- builddfDoseResponse("Ensembl_ID", "ENSMUSG00000032315", dbconn)
  expected_colnames <- c("Project_ID", "Longname", "Dose", "FoldChange", "Species_common",
                         "Sex", "Organ_name", "P1t", "PMID", "GEO", "DesignType", "Chemical_Name",
                         "Assay_Name", "Strain_Name")
  expect_equal(colnames(df), expected_colnames)
...
test_that("multiple genes - Ensembl_ID", {
  expect_error(builddfDoseResponse("Ensembl_ID", c("ENSMUSG00000032315", "ENSMUSG00000032310"), dbconn))
})

# Test with no genes - Ensembl_ID
test_that("no genes - Ensembl_ID", {
  df <- builddfDoseResponse("Ensembl_ID", "", dbconn)
  expect_equal(nrow(df), 0)
})

# Test with single gene - NCBI_ID
test_that("single gene - NCBI_ID", {
  df <- builddfDoseResponse("NCBI_ID", "13076", dbconn)
  expected_colnames <- c("Project_ID", "Longname", "Dose", "FoldChange", "Species_common",
                         "Sex", "Organ_name", "P1t", "PMID", "GEO", "DesignType", "Chemical_Name",
                         "Assay_Name", "Strain_Name")
  expect_equal(colnames(df), expected_colnames)
...
test_that("multiple genes - NCBI_ID", {
  expect_error(builddfDoseResponse("NCBI_ID", c("13076", "13077"), dbconn),
               "More than one gene selected.")
})

# Test with no genes - NCBI_ID
test_that("no genes - NCBI_ID", {
  df <- builddfDoseResponse("NCBI_ID", "", dbconn)
  expect_equal(nrow(df), 0)
})
